...
void setup()
{
  ...
  delay(1800);
  aaniRaja=analogRead(mikrofoniPinni)+0.15*analogRead(mikrofoniPinni);// raja 125%
  valoRaja=analogRead(valoPinni1)-0.15*analogRead(valoPinni1);//raja 75%
...
void loop()
{
  ...
  switch(moodi){
      ...
      case 'e':
      sarjaLiikenne(false, 1, 1000);
      break;

      case 'f':
      sarjaLiikenne(false, 2, 1000);
      break;

      case 'g':
      sarjaLiikenne(false, 3, 1000);
      break;
...
void sarjaLiikenne(bool alustus, int mod, unsigned int viive){
  ...
  switch(mod){
    ...
    case 3:
    Serial.println(analogRead(valoPinni2));
    break;
  }
